...
__device__ glm::vec3 computeColorFromSH_indexed(int idx, int deg, int max_coeffs, const glm::vec3* means, glm::vec3 campos, const float* shs, bool* clamped, const int64_t* sh_indices)
{
	...
	const int64_t* sh_indices)
{
	...
	{
		// TODO this does not be done for every splat... (from compressed 3d)
		computeCov3D_indexed(scales[idx], scale_modifier, rotations[idx], cov3Ds +  idx * 6);
		cov3D = cov3Ds + idx * 6;
	}
	...
	{
		glm::vec3 result = computeColorFromSH_indexed(idx, D, M, (glm::vec3*)orig_points, *cam_pos, shs, clamped,sh_indices);
